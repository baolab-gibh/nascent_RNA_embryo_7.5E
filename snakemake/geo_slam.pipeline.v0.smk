...
apptainer_image = pathlib.Path("~/tools/containers/images/vasaslam_seq.sif").expanduser()
...
# batches = '20240321_Lib_Embryo' # DONE
# batches = '20240321_Lib_20ng' # DONE
# batches = '240612_Lib_28region_purified' # DONE
# batches = 'E200022670-IAA_mix' #DONE

# batches = '240620_Lib_38region' # DONE
# batches = '240409_Lib_embryo' # DONE
...
rule s00_index_genome_star:
  ...
  shell:
    '''
    apptainer exec {apptainer_image} STAR --runMode genomeGenerate \
...
rule s01_demultiplexing:
  ...
  shell:
    '''
    apptainer exec {apptainer_image} fastq-multx -b -B {input.sample_info_tab} {input.read_1} {input.read_2} -o {params.output_dir}/%.R1.fq.gz -o {params.output_dir}/%.R2.fq.gz
    readlink -f {params.output_dir}/*.fq.gz | xargs -n2 echo | grep -v unmatched > {output.dm_file_list}
    '''
...
rule s02_quality_control:
  ...
  shell:
    ...
    apptainer exec {apptainer_image} fastp -w {resources.cpus_per_task} \
        -i ${{raw_r1}} -I ${{raw_r2}} \
    ...
    apptainer exec {apptainer_image} fastqc -q -t {resources.cpus_per_task} -o {params.out_dir} ${{raw_r1}} ${{raw_r2}}
    apptainer exec {apptainer_image} fastqc -q -t {resources.cpus_per_task} -o {params.out_dir} {output.clean_merged_fq} {output.clean_unmerged_r1} {output.clean_unmerged_r2}
    '''
...
rule s04_align_reads:
  ...
  shell:
    ...
    apptainer exec {apptainer_image} STAR --runMode alignReads \
      --twopassMode Basic \
    ...
    apptainer exec {apptainer_image} samtools merge --no-PG \
      -o {output.bam_file} \
      {params.pe_out_prefix}Aligned.sortedByCoord.out.bam {params.se_out_prefix}Aligned.sortedByCoord.out.bam 

    echo "Creating index ..."
    apptainer exec {apptainer_image} samtools index -b -@ {resources.cpus_per_task} {output.bam_file}
...
rule s05_remove_duplicates:
  ...
  shell:
    '''
    mkdir -p {params.out_dir}
    apptainer exec {apptainer_image} umi_tools dedup --method unique --stdin={input.bam_file} --stdout={output.bam_file} --umi-separator=':'
    apptainer exec {apptainer_image} samtools index {output.bam_file}
    '''
...
rule s06_estimate_mapping_quality:
  ...
  shell:
    '''
    mkdir -p {params.out_dir}
    apptainer exec {apptainer_image} qualimap --java-mem-size=8G rnaseq \
      --sorted \
      -bam {input.bam_file} \
...
rule s06_quantify_transcripts:
  ...
  shell:
    '''
    mkdir -p {params.out_dir}
    apptainer exec {apptainer_image} featureCounts -p -t transcript -g transcript_id --extraAttributes gene_id,gene_name -T {resources.cpus_per_task} \
...
rule s06_quantify_enhancer_rna:
  ...
  shell:
    '''
    mkdir -p {params.out_dir}
    apptainer exec {apptainer_image} featureCounts -p -t enhancer -g enhancer_region -T {resources.cpus_per_task} \
...
rule s06_add_cell_barcode:
  ...
  shell:
    '''
    apptainer exec {apptainer_image} samtools view -h -@ {resources.cpus_per_task} {input.bam_file} \
    ...
    apptainer exec {apptainer_image} samtools index -@ {resources.cpus_per_task} {output.bam_file}
    '''
...
rule s07_quantify_spliced_reads:
  ...
  shell:
    '''
    mkdir -p {params.out_dir}
    apptainer exec {apptainer_image} velocyto run -c -o {params.out_dir} {input.bam_files} {input.gtf_file}
...
rule s06_convert_bam_to_cit:
  ...
  shell:
    '''
    mkdir -p {params.out_dir}
    apptainer exec {apptainer_image} gedi -e Bam2CIT -p {output.cit_file} {input.bam_file}
    apptainer exec {apptainer_image} gedi -e CorrectCIT -p {output.cit_file} {output.cit_corrected_file}
    '''
...
rule s07_merge_cits:
  ...
  shell:
    '''
    mkdir -p {params.out_dir}
    apptainer exec {apptainer_image} gedi -e MergeCIT -p {output.merged_cit} {input.cit_corrected_files} {input.control_cit}
    apptainer exec {apptainer_image} gedi -e ReadCount -p {output.merged_cit} 
...
rule s09_convert_nrna_readcounts_into_10X:
  ...
  output:
    ...
    ntr_barcodes = qt_dir / "slamseq/10X/ntr/barcodes.tsv",
    ntr_matrix = qt_dir / "slamseq/10X/ntr/matrix.mtx",
    ntr_genes = qt_dir / "slamseq/10X/ntr/genes.tsv",
  params:
    total_count_dir = qt_dir / "slamseq/10X/total",
    new_to_total_dir = qt_dir / "slamseq/10X/ntr",
    new_count_dir = qt_dir / "slamseq/10X/new",
  shell:
    '''
    mkdir -p {params.total_count_dir} {params.new_count_dir} {params.new_to_total_dir}; rm -f {output}

    zgrep -m1 Readcount {input} | tr "\\t" "\\n" | grep -E "Readcount$" | sed 's/ Readcount$//g' | tee {output.total_barcodes} | tee {output.ntr_barcodes} > {output.new_barcodes}
    zcat {input} | cut -f2 -d$'\\t' | sed -n '2,$p' | xargs -I% echo -e %"\\t"% | tee {output.total_genes} | tee {output.ntr_genes} > {output.new_genes}
...
BEGIN  {{
  ...
  print "%%MatrixMarket matrix coordinate integer general" >> "{output.ntr_matrix}"
  print "%" >> "{output.ntr_matrix}"
  print N_FT,N_BC,N_FT * N_BC >> "{output.ntr_matrix}"
...
NR >= 2 {{
  ...
  for (iiii in sample_list) {{
    ...
    print NT - 1, iiii, new_to_total_ratio >> "{output.ntr_matrix}"
  }}

  close("{output.total_matrix}"); close("{output.new_matrix}"); close("{output.ntr_matrix}")
}}
EOF
    '''
