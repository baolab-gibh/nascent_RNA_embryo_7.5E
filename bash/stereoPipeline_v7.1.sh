...
# Software check
if [ `command -v apptainer` ]
then
    apptainer=`command -v apptainer`
    echo `date` " apptainer check: pass, and apptainer path is ${apptainer}"
else
    echo `date` " apptainer check: apptainer does not exits, please verify that you have installed apptainer and exported it to your system PATH variable"
    exit
fi

if [[ -n $sif ]]
then
    echo `date` " apptainer image file check: file exist and SIF path is ${sif}"
else
    echo `date` " apptainer image file check: file does not exist, please double check your SIF file is in the current directory or the path given by the option -s is valid."
fi
...
if [[ ! -n "$read2" ]]; then
    ...
    export APPTAINER_BIND=$outDir,$maskDIR,$annoDIR,$refDIR
    /usr/bin/time -v apptainer exec ${sif} splitMask \
...
if [[ $fqType == 'Q40' ]]; then
    for ((i=0;i<=`expr $(echo $fqNumber) - 1`;i++)); do
        ...
        export APPTAINER_BIND=$read1DIR,$read2DIR,$outDir,$maskDIR,$annoDIR,$refDIR
        /usr/bin/time -v apptainer exec ${sif} mapping \
    ...
    for ((i=1;i<=$splitCnt;i++)); do
        if [[ $(echo ${#i}) == '1' ]];then a=0$i; else a=$i;fi
        /usr/bin/time -v apptainer exec ${sif} CIDCount \
        ...
        export APPTAINER_BIND=$read1DIR,$outDir,$maskDIR,$annoDIR,$refDIR
        /usr/bin/time -v apptainer exec ${sif} mapping \
            --outSAMattributes spatial \
            --outSAMtype BAM SortedByCoordinate \
            --genomeDir ${GDir} \
...
export APPTAINER_BIND=$outDir,$maskDIR
if [[ $fqType == 'Q4' ]] && [[ $(echo ${#bcReadsCounts[*]}) > '1' ]]; then
    echo 'Q4'
    /usr/bin/time -v apptainer exec ${sif} merge \
        ${maskFile} \
        $bcReadsCountsStr \
        $barcodeReadsCounts
...
export APPTAINER_BIND=$outDir,$annoDIR,$refDIR
export HDF5_USE_FILE_LOCKING=FALSE
/usr/bin/time -v apptainer exec ${sif} count \
    -i ${starBamsStr} \
...
if [[ -f $imageTarFile ]] && [[ -f $iprFile ]]  && [[ $doCell == "Y" ]]; then
    ...
    imgTarDIR=$(dirname $imageTarFile)
    iprDIR=$(dirname $iprFile)
    export APPTAINER_BIND=$outDir,$imgTarDIR,$iprDIR

    /usr/bin/time -v apptainer exec ${sif} register \
        -i ${imageTarFile} \
        -c ${iprFile} \
    ...
    /usr/bin/time -v apptainer exec ${sif} imageTools ipr2img \
        -i ${imageTarFile} \
        -c ${out_iprFile} \
        -d tissue cell \
        -r True \
    ...
    echo $regTifStr
    echo $regGroupStr
    /usr/bin/time -v apptainer exec ${sif} imageTools img2rpi \
        -i ${regTifStr} \
        -g ${regGroupStr} \
        -b 1 10 50 100 \
...
if [[ -f $imageTarFile ]] && [[ -f $iprFile ]]; then
    ...
    export HDF5_USE_FILE_LOCKING=FALSE
    export APPTAINER_BIND=$outDir
    /usr/bin/time -v apptainer exec ${sif} tissueCut \
    ...
    export HDF5_USE_FILE_LOCKING=FALSE
    /usr/bin/time -v apptainer exec ${sif} cellCut bgef \
    ...
    out_iprFile=$(find ${outDir}/03.register -maxdepth 1 -name \*.ipr | head -1)
    for i in `apptainer exec ${sif} h5dump -n $out_iprFile | grep 'Labeling/' | grep -v 'Labeling/.*/Canvas'|grep -v 'Labeling/.*/Element'|awk '{print$2}'`;
    do
    label=`basename $i`
...
# /usr/bin/time -v apptainer exec ${sif} cellCut view \
#     -s ${SN} \
...
/usr/bin/time -v apptainer exec ${sif} spatialCluster \
  -i ${outDir}/04.tissuecut/${SN}.tissue.gef \
  -s ${binSize} \
	-r ${resolution} \
...
if [[ -f $imageTarFile ]] && [[ -f $iprFile ]] && [[ $doCell == "Y" ]]; then
    ...
    /usr/bin/time -v apptainer exec ${sif} cellCut cgef \
        -i ${outDir}/02.count/${SN}.raw.gef \
        -m ${nucleusMask} \
    ...
    export HDF5_USE_FILE_LOCKING=FALSE
    /usr/bin/time -v apptainer exec ${sif} cellCorrect \
            -i ${outDir}/02.count/${SN}.raw.gef \
            -m ${nucleusMask} \
            -d 10 \
    ...
    /usr/bin/time -v apptainer exec ${sif} cellCluster \
    ...
    /usr/bin/time -v apptainer exec ${sif} cellChunk \
        -i ${outDir}/041.cellcut/${SN}.adjusted.cellbin.gef \
        -o ${outDir}/041.cellcut/
...
/usr/bin/time -v apptainer exec ${sif} saturation \
...
if [[ -n ${out_iprFile} ]] && [[ -e ${out_iprFile} ]] && [[ $doCell == 'Y' ]]; then
    /usr/bin/time -v apptainer exec ${sif} report \
        -m ${bcStatStr} \
        -a ${bcFinalOutStr} \
